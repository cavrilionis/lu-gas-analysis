...
source("01-download_ilr_data.R")
source("02-download_meteolux_data.R")
```

```{r import_data, message = FALSE, warning = FALSE, include = FALSE}
source("03-import-data.R")
```
...
```{r stats_temp_celcius}
summary(df$temp_celcius)
```
...
p1 <- ggplot(df, aes(x = consumption_gwh)) +
  geom_histogram(binwidth = 2 * IQR(df$consumption_gwh) / length(df$consumption_gwh)^(1/3), fill = "lightgrey", color = "grey35") +
  theme_minimal() +
  labs(x = "Monthly gas consumption in GWh",
       y = "Frequency")

p2 <- ggplot(df, aes(x = temp_celcius)) +
  geom_histogram(binwidth = 2 * IQR(df$temp_celcius) / length(df$temp_celcius)^(1/3), fill = "lightgrey", color = "grey35") +
  theme_minimal() +
  labs(x = "Mean monthly temperature in degrees Celcius",
       y = "Frequency")
...
df |>
  ...
  labs(
    x = "Year",
    y = NULL,
    title = "Seasonal subseries plot: Monthly residential gas consumption in GWh"
  ) +
  ylim(0, NA) +
  theme_minimal() +
  theme(
...
ggplot(df, aes(x = temp_celcius, y = consumption_gwh)) +
  ...
  labs(
    ...
    title = "Scatterplot: Mean monthly air tempreture and monthly residential gas consumption in GWh"
  ) + 
  theme_minimal()
```
...
```{r correlation_test}
cor.test(df$consumption_gwh, df$temp_celcius,
         method = "pearson",
...
df |>
  ...
  labs(x = "lag(gas consumption in GWh, k)",
...
df |>
  ...
  autoplot() +
  labs(title = "ACF plot: Monthly gas consumption in GWh",
       x = "Lag in months",
       y = "ACF") +
  theme_minimal()
```
...
### Trend strength and seasonal strength
